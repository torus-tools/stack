    ...
    cloudfront.listDistributions({}, function(err, data) {
      if (err) throw new Error(err.stack);
        ...
        for(let i in items){
          ...
          else if(i === data.DistributionList.Items.length -1) callback(null, null)
        }
      }
    });  
}
...
function bucketExists(domainName, callback) {
   ...
   s3.getBucketPolicyStatus(params, function(err, data) {
    if(err) {
      if(err.code === 'NoSuchBucket') callback(null, false)
      else if(err.code === 'NoSuchBucketPolicy') console.log('Bucket policy doesnt exist')
      else callback(err)
    }
    else {

...
async function deployStack(domain, stack, config, overwrite){
  ...
  if(stackId && JSON.stringify(fullTemplate.template) === templateString) return('no changes detected')
    ...
    if(fullTemplate.existingResources.length > 1){
      ...
      .then(()=>deployParts(domain, stack, partialTemplate, partialStack, fullTemplate, importsTemplate))
    }
    //update or create
    else deployParts(domain, stack, partialTemplate, partialStack, fullTemplate, template)
  }
}

function deployParts(domain, stack, partialTemplate, partialStack, fullTemplate, template){
  let size = 0;

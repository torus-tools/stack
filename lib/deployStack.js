...
async function deployStack(domain, stack, config, content, overwrite, cli){
  ...
  let partialRecords = false
  let fullRecords = false
  if(stack.dns && config.providers.dns === 'aws') {
    if(!stack.cdn) partialRecords = true
    else if(stack.cdn && config.providers.cdn === 'aws') fullRecords = true
  }

  const partTemplate = await generateTemplate(domain, partialStack, config, template, partialRecords, overwrite).catch(err => {throw new Error(err)})
  const partialTemplate = JSON.parse(JSON.stringify(partTemplate))
  const fullTemplate = await generateTemplate(domain, stack, config, template, fullRecords ,overwrite).catch(err => {throw new Error(err)})

        ...
        if(err) console.log(err)
          ...
          executeChangeSet(stackName, data.ChangeSetName, (err,data) => {
            if(err) throw new Error(err)
              ...
              if(https){
                validateCertificates(stackName, route53, (err,data) => {
                  if(err) throw new Error(err)
                  else importResources(stackName, template, existingResources, callback)
                })
              }
              else {
                importResources(stackName, template, existingResources, callback)
              }
            }
          })
        }
...
function validateCertificates(stackName, route53, callback){
  acmCertificate.getCertificateArn(stackName)
  .then((arn) => {
    acmCertificate.describeCertificate(arn)
    .then((data) => {
      if(route53) {
        acmCertificate.validateCertificate(data.cName, data.cValue, stackName)
        .then(callback(null, 'valid')).catch((err)=>{throw new Error(err)})  
      }
      else{
        let msg = 
        `
...
function importResources(stackName, template, existingResources, callback){

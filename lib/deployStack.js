...
module.exports = function deployTemplate(domain, template, existingResources, https, ){
        ...
        if(err) console.log(err)
          ...
          executeChangeSet(stackName, data.ChangeSetName, (err,data) => {
            if(err) throw new Error(err)
              ...
              newHostedZone(domain, stackName)
              if(https){
...
function newHostedZone(domain, stackName){
  if(route53){
    if(!hostedZoneExists(domain)){
      var params = {

  ...
  return new Promise((resolve, reject) => {
    ...
    for(let key in partialStack) {
      ...
      if(partialStack[key] && !template || partialStack[key] && !template.Resources[stackResources[key]]) {
        ...
        cli? cli.action.start('deploying the partial template') : console.log('deploying the partial template . . .')
        //console.log(partialStack, stackResources[key])
        //console.log('PARTIAL TEMPLATE EXECUTING', JSON.stringify(partialTemplate))
        deployTemplate(domain, partialTemplate).then(() => {
          cli? cli.action.stop(): console.log('finished deploying the partial template')
...
function deployFull(domain, stack, config, fullTemplate, partialTemplate, content, cli){
  return new Promise((resolve, reject) => {
    ...
    if(stack.dns && config.providers.dns !== config.providers.domain){
      ...
      domains[config.providers.dns].getNameservers(domain).then(nameservers => {
        if(supported_providers.domain.includes(config.providers.domain)) {
          ...
          cli? cli.action.start('transfering DNS Nameservers . . .') : console.log('transfering DNS Nameservers . . .')
          //console.log('DOMAIN ', domain)
          //console.log('NAMESERVERS ', nameservers)
          //console.log('PROVIDER DOMAIN ', config.providers.domain)
          domains[config.providers.domain].updateNameservers(domain, nameservers).then(data=>{
            //console.log('DATA RESPONSE ', data)
            cli? cli.action.stop(): console.log('finished transfering nameservers')
            arr.push(2)
            if(arr.length >= 3) resolve(url)
          }).catch(err => reject(err))
        }
    ...
    if(JSON.stringify(fullTemplate) !== JSON.stringify(partialTemplate)){
      cli? cli.action.start('Deploying the full template') : console.log('Deploying the full template . . .')
      //console.log('FULL TEMPLATE EXECUTING ', JSON.stringify(fullTemplate))
      deployTemplate(domain, fullTemplate).then(()=>{
        cli? cli.action.stop(): console.log('finished deploying the full template')
          ...
          if(config.providers['dns'] !== 'aws') {
            if(supported_providers.dns.includes(config.providers.dns)){
              ...
              domains[config.providers.dns].upsertRecords(domain, records).then(()=>{
                if(recordReroute) {
                  cli? cli.action.stop(): console.log('finished creating DNS records')
                  arr.push(3)
                  if(arr.length >= 3) resolve(url)
                }

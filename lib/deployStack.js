...
async function deployStack(domain, template, existingResources, cdn, https, route53){
  ...
  let changeSetName = await createChangeSet(stackName, template, existingResources, false)
  cloudformation.waitFor('changeSetCreateComplete', {ChangeSetName: changeSetName, StackName: stackName}).promise()
  .then(()=> {
    cloudformation.executeChangeSet({ChangeSetName: changeSetName, StackName: stackName}).promise()

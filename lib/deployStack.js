...
async function deployStack(domain, stack, config, content, overwrite, local, cli){
  ...
  var partTemplate = {}
  if(local) partTemplate = JSON.parse(fs.readFileSync('./torus/template.json', utf8))
  else partTemplate = await generateTemplate(domain, partialStack, config, template, overwrite).catch(err => {throw new Error(err)})
  var partialTemplate = JSON.parse(JSON.stringify(partTemplate))
  var fullTemplate = partialTemplate
  if(!local) fullTemplate = await generateTemplate(domain, stack, config, template, overwrite).catch(err => {throw new Error(err)})

  if(partialTemplate && fullTemplate) console.log('finished generating templates')
  
  console.log(JSON.stringify('PARTIAL TEMPLATE ', partialTemplate))
  console.log(JSON.stringify('FULL TEMPLATE ', fullTemplate))

...
module.exports = function genTemplate(domain, stack, config, template, records, overwrite){
  return new Promise((resolve, reject) => {
    if(domainIsValid(domain)){
      ...
      let new_temp = template? JSON.parse(JSON.stringify(template)) : initialTemplate
      ...
      if(stack.ssl && config.providers.ssl === 'aws'){
        ...
        new_temp.Resources.CloudFrontDist.Properties.DistributionConfig.DefaultCacheBehavior["ViewerProtocolPolicy"] = "redirect-to-https"

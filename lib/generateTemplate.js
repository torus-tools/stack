...
module.exports = function genTemplate(domain, index, error, cdn, route53, https, callback){
  ...
  return new Promise((resolve, reject) => {
    //check if root bucket already exists
    checkResource.bucketExists(domain).then((data) =>{
      if(data) addToResources(ResourcesToImport, 'RootBucket', domain, 'AWS::S3::Bucket');
      else template.Resources["RootBucket"] = RootBucket;
    }).then(checkResource.bucketExists(wwwDomain).then((data) => {
      ...
      if(!cdn){
        ...
        //.then(resolve(Response(domain, template, ResourcesToImport))) 
      }
      else {
        checkResource.distributionExists(domain).then((data)=>{
          if(data){
        ...
        .then(()=>{
          ...
          if(route53){
            checkResource.hostedZoneExists(domain).then((data)=> {
              ...
              resolve(Response(domain, template, ResourcesToImport))
            })
          }
          else {
            //console.log('HELLOooooo')

...
module.exports = function genTemplate(domain, stack, config, template, records, overwrite){
  return new Promise((resolve, reject) => {
    if(domainIsValid(domain)){
      ...
      if(stack.bucket && config.providers.bucket === 'aws') template.Resources['BucketPolicy'] = defaults['BucketPolicy']
      if(records) template.Resources['RecordSet'] = defaults['RecordSet']

      if(overwrite) {
        for(let key in stack) if(stack[key] && config.providers[key] === 'aws'){
      ...
      for(let resource in stack) if(stack[resource] && resourceExists[stack[resource]] && config.providers[resource] === 'aws'){
        ...
        resourceExists[stack[resource]](domain).then(data => {
          if(importables[stack[resource]] && data && data.id) addToResources(ResourcesToImport, stack[resource], data.id, template.Resources[stack[resource]].Type, importables[stack[resource]])
          .then(()=>{

...
module.exports = async function genTemplate(domain, index, error, www, cdn, route53, https, action){
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if(action !== 'import') template.Resources["BucketPolicy"] = BucketPolicy;
      if(!template.Resources.RootBucket){
        await checkResource.bucketExists(domain).then((data) =>{
          if(data) {
            addToResources(ResourcesToImport, 'RootBucket', domain, 'AWS::S3::Bucket', 'BucketName');
            //addToResources(ResourcesToImport, 'BucketPolicy', domain, 'AWS::S3::BucketPolicy')
          }
      ...
      if(www && !template.Resources.WwwBucket){
        await checkResource.bucketExists(wwwDomain).then((data) => {
          if(data) addToResources(ResourcesToImport, 'WwwBucket', wwwDomain, 'AWS::S3::Bucket', 'BucketName');
      ...
      if(route53 && !template.Resources.HostedZone){
        await checkResource.hostedZoneExists(domain).then((data)=> {
          if(data){
            HostedZoneRef = data;
            addToResources(ResourcesToImport, 'HostedZone', HostedZoneRef, 'AWS::Route53::HostedZone', 'HostedZoneId')
            //addToResources(ResourcesToImport, 'RecordSet', HostedZoneRef, 'AWS::Route53::RecordSetGroup')
          }

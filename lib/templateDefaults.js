...
function templateDefaults(domain, stack, config) {
  ...
  return {
    "RootBucket":{
      ...
      "Properties": {
        ...
        "WebsiteConfiguration": {
          "ErrorDocument" : config.options.error,
          "IndexDocument" : config.options.index
        },
    ...
    "CloudFrontDist":{
      ...
      "Properties": {
        "DistributionConfig":{
          ...
          "DefaultRootObject" : config.options.index,
          "Enabled" : true,
          "HttpVersion":"http2",
          "Origins" : [
            {
    ...
    //add to importables as false
    //correct the RoleArn property. create a role for the codepipeline
    //correct the Name property cant use the dot in the domain
    "CodePipeline": {
